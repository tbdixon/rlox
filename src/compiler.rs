...
impl Compiler {
    ...
    fn end_function(self) {
        ...
        unsafe {
            ...
            let function_idx = enclosing.create_constant(Value::from(self.function));
        ...
        match self.resolve_local(&identifier) {
            ...
            None => match self.resolve_upvalue(&identifier) {
                ...
                None => {
                    ...
                    arg = self.create_constant(Value::from(identifier));
    ...
    fn string(&mut self) {
        ...
        let const_idx = self.create_constant(Value::from(string));
        self.emit_read_constant(const_idx);
    }
    ...
    fn parse_variable(&mut self) -> usize {
        ...
        if self.depth == 0 {
            let const_idx = self.create_constant(Value::from(variable_name));
            self.emit_read_constant(const_idx);
            const_idx
        } else {
